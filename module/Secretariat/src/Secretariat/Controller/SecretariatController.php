...
/*****************************************************************************************************************************/
	...
	public function ajouterPatientAction() {
		...
		$data = array('nationalite_origine' => 'S√©n√©gal', 'nationalite_actuelle' => 'S√©n√©gal');
		...
		$html ='<table style="width: 100%;" >
			  ...
			  foreach ($listeDemandesAnalyses as $listeDemandes){
			  	...
			  	$html .="<tr style='height: 25px; width:100%;'>
			  	           <td id='numerof'>
      					       <hass><img style='padding-left: 10px; cursor: pointer;' class='info_secretaire".$listeDemandes['iddemande']."' src='../images_icons/info_infirmier.png' title='envoy√© par ".$infosSecretaire['prenom'].' '.$infosSecretaire['nom']."' /></hass>
			  			   </td>
      					   <td>". $date ."</td>
			  	           <td>";
	...
	public function impressionResultatsAnalysesDemandeesAction()
	{
		...
		//Recuperation de la liste des analyses pour lesquelles les rÈsultats sont dÈj‡ renseignÈs et validÈs
		...
		for($j = 0 , $i = 0 ; $i < count($listeResultats) ; $i++ ){
			...
			if($idanalyse == 1){ //NFS
				...
				//RecupÈration des antÈriorites  ----- RÈcupÈration des antÈrioritÈs
		...
		if($analysesImmunoHemato || $analysesCytologie || $analysesHemostase || $analysesMetabolismeGlucidique ||
		   ...
		   $analysesMetabolismeFer || $analysesMetabolismeProtidique){
			...
			//GESTION DES ANALYSES DU TYPAGE (HelectrophorËse)
			//GESTION DES ANALYSES DU TYPAGE (HelectrophorËse)
	...
	*/
	...
	public function impressionResultatsAnalysesDemandeesAction()
	{
		...
		for($j = 0 , $i = 0 ; $i < count($listeResultats) ; $i++ ){
			...
			elseif($idanalyse == 53){ //CRP ou C. ProtÈine RÈactive
				$analysesDemandees  [$j++] = $listeResultats[$i];
				$analysesSerologie  [] = 53;
				$resultatsAnalysesDemandees[53] = $this->getResultatDemandeAnalyseTable()->getValeursCrp($iddemande);
			}
		...
		if($analysesNFS){
			...
			/*
			 * Envoie des donnÈes pour affichage
			*/
			$pdf->affichageResultatAnalyseNFS();
		}
	
		//========= CrÈaton des autres pages ========
		//========= CrÈaton des autres pages ========
		//========= CrÈaton des autres pages ========
		if($analysesImmunoHemato || $analysesCytologie || $analysesHemostase || $analysesMetabolismeGlucidique ||
		$analysesBilanLipidique || $analysesBilanHepatique || $analysesBilanRenal || $analysesSerologie || $analysesTypageHemoProteine ||
		...
		//========= CrÈaton de la page SÈrologie HIV ========
		//========= CrÈaton de la page SÈrologie HIV ========
		//========= CrÈaton de la page SÈrologie HIV ========
		...
		//Afficher le document contenant les diffÈrentes pages
		//Afficher le document contenant les diffÈrentes pages
		//Afficher le document contenant les diffÈrentes pages
		$pdf->Output('I');
	...
	//Une fois que le patient est appelÈ 
	//Une fois que le patient est appelÈ
	//Une fois que le patient est appelÈ
	...
	//Si le patient n'est pas appelÈ 
	...
	public function verifierPatientExistantAction() {
		...
		if($verifPatientExiste[0] == 1){
			$html .= "<script>
					   $('#patientInfosSupp').html('". str_replace("'", "\'", "<img style='cursor: pointer; margin-top: 10px;' src='../images_icons/voir2.png' title='".$verifPatientExiste[1][0]['patientPrenom']." ".$verifPatientExiste[1][0]['patientNom']." est n√© a ".$verifPatientExiste[1][0]['patientLieuNaissance']." et habite a ".$verifPatientExiste[1][0]['patientAdresse']."' />")."');
