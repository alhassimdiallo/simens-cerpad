...
class PruneLevel(object):
  def __init__(self, selected_ex_inters, compute_height, prop_2_box):
  ...
  def __repr__(self):
    return """PruneLevel(%s, %s, %s, %s)""" % (self.min_nb_gene_per_box, self.nb_box, self.box_nbs, self.box_includes)
...
def gene_lists_2_html_page(genes, gene_lists, height_unit = None, max_nb_box = 64, static_url = "", height_factor = 30.0, detail_on_demand = True, dynamic_clustering = True, nb_tested_solution = 3000):
  relations = []
  ex_inters = []
  subset_2_ex_inter = {  }
  prune_levels = []
  ...
  if len(gene_lists) <= 15:
    for gene_list in gene_lists:
      gene_list.element = Element(None, "%s (%s)" % (gene_list.name, len(gene_list.genes)),
                                  color   = (0.85, 0.85, 0.85))
  else:
    for gene_list in gene_lists:
      gene_list.element = Element(None, "%s (%s)" % (gene_list.name, len(gene_list.genes)),
                                  color   = (0.85, 0.85, 0.85), rotate = 10.0)
  ...
  if len(gene_lists) < 10:
    ...
    #order = [gene_list.element for gene_list in gene_lists]
...
"""
  ...
  prune_levels.append(PruneLevel(selected_ex_inters, compute_height, prop_2_box))

  if prune_levels[-1].nb_box == len(ex_inters):
    prune_levels[-1].min_nb_gene_per_box = 1

  if dynamic_clustering:
    while prune_levels[-1].nb_box > 1 + len(gene_lists):
      #selected_ex_inters = prune(selected_ex_inters, prune_levels[-1].nb_box - 1)
...
class PruneLevel(object):
  def __init__(self, min_nb_gene_per_box, nb_box, box_nbs, box_includes):
...
def on_change_min_nb_gene_per_box(delta):
  ...
  document["min_nb_gene_per_box"].innerHTML = str(prune_level.min_nb_gene_per_box)
