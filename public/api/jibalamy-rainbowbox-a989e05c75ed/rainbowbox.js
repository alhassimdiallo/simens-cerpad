...
function move_boxes1(parent, update_height) {
  ...
  if (sel_boxes_id == undefined) { // Non interactive
    ...
    for (var c in boxes) {
      ...
      if (box_element.offsetHeight > 20) move_box(c, ys, box_y, 3);
      else                               move_box(c, ys, box_y, 1);
    }
    */
    for (var c in boxes) {
      c = boxes[c];
      move_box(c, ys, box_y, 2);
    }
  }
  else { // Interactive
    for (var c in boxes) {
  ...
  for (var c in boxes) {
    ...
    //if ((box_element.style.opacity != 0) & (box_element.style.display != "none")) {
...
function on_box_enter(element, event) {
  popup_enter(element, event);
}
function on_box_exit(element, event) {
  popup_exit(element, event);
}
