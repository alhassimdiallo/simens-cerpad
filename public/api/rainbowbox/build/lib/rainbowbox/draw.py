...
#from model import *
#from model import _get_property_weight
...
class HTMLPage(object):
  ...
  def get_script(self):
    script  = """parent_id_2_height_id      = %s;\n""" % repr(self.parent_id_2_height_id)
...
%s
...
</html>""" % (" brython(1);" if self.has_brython else "", self.get_script(), self.get_html())
    return html
  ...
  def show(self, sleep_time = 5.0):
    ...
    for filename in ["rainbowbox.css", "rainbowbox.js"]:
      f = open(os.path.join(tmpdir.name, filename), "w")
      f.write(open(os.path.join(os.path.dirname(__file__), filename)).read())
      f.close()
  ...
  def html_popup(self, label, details, tag = "div", classname = "", style = ""):
    if not details:
      if classname:
        ...
        return label
    return """<%s %s%sonMouseEnter="this.lastChild.style.display='block';" onMouseOut="this.lastChild.style.display='none';">%s<div class="rainbowbox_popup">%s</div></%s>""" % (tag, classname and ('class="%s" ' % classname) or "", style and ('style="%s" ' % style) or "", label, details, tag)
  
  def rainbowbox(self, relations, elements = None, order = None, colors = None, keep_previous_colors = False, empty_text = "(empty)", use_element_details_in_relation = False, optimize_box_order = True, sort_relation_by_color = False, interactive_boxes = False, black_and_white = False, property_order = None, versus = False, hideable_element = False, clickable_properties = False):
    ...
    height_id = self.parent_id_2_height_id[parent_id] = self.gen_id()
    html += """<tr>\n"""
    for i in range(len(order)):
    ...
    for box in boxes:
      ...
      if not box_color:
        if isinstance(box.weight, set):
          ...
          if black_and_white:
            ...
            #box_color = rgb_mean([colors[i] for i in box.ids])
      ...
      #if box_x + box_w == len(cols): extra_style += " padding-right: 0px;"
      ...
      if box_has_hatch:
        ...
        background  = """background-image: repeating-linear-gradient(135deg, %s, %s 12px, %s 12px, %s 20px);""" % (background_color, background_color, hatch_color, hatch_color)
      else:
      ...
      if holes:
        ...
        for hole_x, hole_w in holes:
          for j in [1, 2]:
            #html += """<div class="rainbowbox_hole%s" style="left: %s%%; width: %s%%;%s">\n""" % (j, 100.0 * (hole_x - box_x) / box_w, 100.0 * hole_w / box_w, extra[j-1])
      ...
      def open_div(property, has_color, has_hatch, label = None, details = None):
        nonlocal div_opened, has_region_div, previous_colors, n_box_label, html
        if not div_opened:
          div_opened = True
          ...
          if   has_color:
            ...
            for i in range(box_x_i, box_x_i + box_w_i):
              ...
              if hatch:
                #if i == region_x:
                #  div_background += """, rgba(%s,%s,%s,0.6) %s%%, transparent %s%%""" % (int(255 * box_color[0]), int(255 * box_color[1]), int(255 * box_color[2]), x1, x2)
                #else:
          ...
          #if weighted:
          #  html += """<div class="rainbowbox_box_label_weighted" style="%s">\n""" % div_background
          #else:
          #  html += """<div class="rainbowbox_box_label" style="%s">\n""" % div_background
          if details:
            extra = ''' onMouseEnter="on_box_enter(this, event);" onMouseOut="on_box_exit(this, event);"'''
            #extra = ''' onClick="this.firstChild.style.display='block';"'''
          else:
            extra = ""
            
          if weighted:
            html += """<div class="rainbowbox_box_label_weighted" style="%s"%s>""" % (div_background, extra)
          else:
            html += """<div class="rainbowbox_box_label" style="%s"%s>""" % (div_background, extra)
            
          if details:
            html += """<div class="rainbowbox_popup" style="display: none; top: 1em;">%s</div>\n""" % details
          else:
            html += "\n"
      ...
      def close_div():
        nonlocal div_opened, has_region_div, n_box_label, html
        if div_opened:
          if has_region_div:
            has_region_div = False
            html += """</div>"""
        ...
        for property in groupped_properties:
          ...
          if (box.weight is None) and (len(properties) > 15) and (len(groupped_properties) > 1) and ((len(groupped_properties[0].ids) > 1) or (len(order) < 6)) and (not "<div" in property.label):
            ...
            if   box.weight is None: style = ""
              ...
              #style = ""
              #label = """<div style="height: %sem; line-height: %sem; font-size: 80%%; margin-top: 0.0em; margin-bottom: 0.0em;">%s</div>""" % (box.min_weight, box.min_weight, label)
              ...
              #style = "margin-top: 0.0em; margin-bottom: 0.0em; height: %sem; display: table;" % box.min_weight
              #label = """<div style="display: table-cell; vertical-align: middle;">%s</div>""" % label
              #style = "margin-top: 0.0em; margin-bottom: 0.0em; height: %sem; display: table;" % box.min_weight
              #label = """<div class="rainbowbox_ellipsis" style="display: table-cell; vertical-align: middle;">%s</div>""" % label
              #style = "margin-top: 0.0em; margin-bottom: 0.0em; height: %sem;" % box.min_weight
              #label = """<div style="vertical-align: middle;"><div class="rainbowbox_ellipsis" style="display: block;">%s</div></div>""" % label
              #style = "margin-top: 0.0em; margin-bottom: 0.0em; height: %sem;" % box.min_weight
              #label = label
            ...
            #html += self.html_popup(label, details, "div", style = style) + """\n"""
    ...
    html += """</div>\n""" # rainbowbox_parent_1
    html += """</div>\n""" # rainbowbox_parent_0
    html += """<div class="rainbowbox_bottom"></div>\n"""
