...
</div>""" % (self.rotate, self.rotate, self.rotate / 2.0, self.rotate / 2.0, self.label)
    else:
      return self.label
...
class Property(object):
  def __init__(self, group, label, order_key = 0, details = "", weight = None, start_new_box = False, at_bottom = False, color = None, ellipsis = True, rotate = None):
    self.group         = group
    self.label         = label
  ...
  def clone(self): return Property(self.group, self.label, self.order_key, self.details, self.weight, self.start_new_box, self.at_bottom, self.color)
