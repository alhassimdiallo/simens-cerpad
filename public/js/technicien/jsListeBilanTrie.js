    	...
    	var asInitVals = new Array();
   	    ...
   	    ( {
   		  ...
   		  "oLanguage": {
   			   ...
   			   "sAjaxSource": ""+tabUrl[0]+"public/technicien/liste-bilans-tries-ajax", 
    ...
    function modifierPrelevementTrie(idpatient, idfacturation)
    {
        ...
        var chemin = tabUrl[0]+'public/technicien/vue-modifier-liste-prelevement-tri';
        $.ajax({
            type: 'POST',
            url: chemin ,
            ...
            success: function(data) {
            	...
            	$('#titre span').html('MODIFIER LES TRIS DES PRELEVEMENTS');
            	var result = jQuery.parseJSON(data); 
            	$("#contenu").fadeOut(function(){ $("#vue_patient").html(result); $("#interfaceListePrelevement").fadeIn("fast"); }); 
...
function popupPrelevementPrecedentAfficher(){
    ...
    //FEUILLE DE PAILLASSE Choix 1 (Par Analyse)
    //FEUILLE DE PAILLASSE Choix 1 (Par Analyse)
    function genererFeuillePaillasseChoix1(){
    	var vart = tabUrl[0]+"public/technicien/impression-feuille-paillasse";
        
    	var FormulaireImprimerFeuillePaillasse = document.getElementById("FormulaireImprimerFeuillePaillasse");
    	FormulaireImprimerFeuillePaillasse.setAttribute("action", vart);
    	FormulaireImprimerFeuillePaillasse.setAttribute("method", "POST");
    	FormulaireImprimerFeuillePaillasse.setAttribute("target", "_blank");
        
    	$("#ImprimerFeuillePaillasse").trigger('click');
    }
    
    //FEUILLE DE PAILLASSE Choix 2 (Par Patient)
    //FEUILLE DE PAILLASSE Choix 2 (Par Patient)
    function genererFeuillePaillasseChoix2(){
    	var vart = tabUrl[0]+"public/technicien/impression-feuille-paillasse-par-patient";
    ...
    function popupChoixFeuillePaillasse(){

    	$( "#choixFeuillePaillasse" ).dialog({
    		resizable: false,
    		height: 100,
    		width: 345,
    		autoOpen: false,
    	...
    	if(entreUneFois == 0){ 
    		$('#RechercheParAnalyse').click(function(){
    	    	$("#choixFeuillePaillasse").dialog('close');
    	    	genererFeuillePaillasseChoix1();
    	    });
    		$('#RechercheParPatient').click(function(){
    			$("#choixFeuillePaillasse").dialog('close');
    			genererFeuillePaillasseChoix2();
    	    });
    		entreUneFois = 1;
    	}
    ...
    var entreUneFois = 0;
    function getChoixFeuillePaillasse(){
    	popupChoixFeuillePaillasse();
		$("#choixFeuillePaillasse").dialog('open');
    }
    
    
    function modifiertriPrelevementRepris(idpatient, idfacturation){
    	//POUR LE MOMENT RIEN A MODIFIER
    	//POUR LE MOMENT RIEN A MODIFIER
    	//POUR LE MOMENT RIEN A MODIFIER
