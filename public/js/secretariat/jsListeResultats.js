    ...
    function initialisation()
    {
    	 ...
    	 ( {
    		 ...
    		 "sAjaxSource":  tabUrl[0]+"public/secretariat/liste-resultats-ajax",
   ...
   function clickRowHandler() 
   {
    	...
    	$('#patient tbody tr').contextmenu({
    		...
    		onItem: function (context, e) {
    			...
    			if($(e.target).text() == 'Visualiser' || $(e.target).is('#visualiserCTX')){
    				...
    				if($(e.target).text() == 'Analyses valid̩es' || $(e.target).is('#analysesValideesCTX')){
    					
    					if(id){ listeResultatsAnalyses(id); }
    				}
    ...
    function listeResultatsAnalyses(idpatient){
    	
        var chemin = tabUrl[0]+'public/secretariat/get-informations-resultats-analyses-patient';
        $.ajax({
            type: 'POST',
            url: chemin ,
    ...
    function vueListeAnalysesDemande(iddemande){
    	var chemin = tabUrl[0]+'public/secretariat/get-resultats-liste-analyses-demandees';
        $.ajax({
            type: 'POST',
            url: chemin ,
    ...
    {
    	if(iddemande){
    		var vart = tabUrl[0]+'public/secretariat/impression-resultats-analyses-demandees';
