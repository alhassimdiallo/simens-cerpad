...
function chargementModificationAnalyses (listeAnalysesDemandees, tabListeAnalysesParType) {
	...
	for(var index = 0 ; index < listeAnalysesDemandees.length ; index++){
		...
		var result = listeAnalysesDemandees[index]['result'];
		if(result == 1){ $isupp++;
...
function montantTotal(){ 
	...
	if(somme != 0){
		$("#montantTotal span").html("<div style='float: left; margin-top: -5px; margin-right: 10px;'> Montant total: </div> <div style='margin-top: -8px; margin-right: 50px; font-size: 20px; font-weight: bold; width: 120px; float: left;'>"+prixMill(somme)+" frs </div> ");
	}else {
